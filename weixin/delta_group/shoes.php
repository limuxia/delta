...
$back = mysql_query("SELECT COUNT(*) AS nb_products FROM shoes"); 
...
$req = mysql_query('SELECT id,name,size,norm,description,photo FROM shoes ORDER BY id ASC LIMIT ' . $firstmessage . ', ' . $nbprodbypage); 
...
while ($data = mysql_fetch_array($req)){ 
 ...
 if($data['norm']=='pictures/norm/shoes/')
 {
 	echo '<img src="';
...
for ($i = 1 ; $i < $num ; $i++) { 
echo '<a href="content2.php?content=shoes&num=' . $i . '">'. $i .'</a> &nbsp;|&nbsp; '; 
} 
echo '<a href="content2.php?content=shoes&num=' . $num . '"><b><font size="+1">'. $num .'</font></b></a> &nbsp;|&nbsp; ';
