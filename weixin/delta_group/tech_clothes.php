...
$back = mysql_query("SELECT COUNT(*) AS nb_products FROM tec_clothes"); 
...
$req = mysql_query('SELECT id,name,size,norm,description,photo FROM tec_clothes ORDER BY id DESC LIMIT ' . $firstmessage . ', ' . $nbprodbypage); 
...
while ($data = mysql_fetch_array($req)){ 
 ...
 if($data['norm']=='pictures/norm/tec_clothes/')
 {
 	echo '<img src="../';
...
for ($i = 1 ; $i < $num ; $i++) { 
echo '<a href="content2.php?content=tech_clothes&num=' . $i . '">'. $i .'</a> &nbsp;|&nbsp; '; 
} 
echo '<a href="content2.php?content=tech_clothes&num=' . $num . '"><b><font size="+1">'. $num .'</font></b></a> &nbsp;|&nbsp; ';
