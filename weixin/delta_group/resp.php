...
$req = mysql_query('SELECT id,name,norm,description,photo FROM respiratory ORDER BY id DESC LIMIT ' . $firstmessage . ', ' . $nbprodbypage); 
...
while ($data = mysql_fetch_array($req)){ 
 ...
 if($data['norm']=='pictures/norm/respiratory/')
 {
 	echo '<img src="../';
...
for ($i = 1 ; $i < $num ; $i++) { 
echo '<a href="content2.php?content=resp&num=' . $i . '">'. $i .'</a> &nbsp;|&nbsp; '; 
} 
echo '<a href="content2.php?content=resp&num=' . $num . '"><b><font size="+1">'. $num .'</font></b></a> &nbsp;|&nbsp; ';
