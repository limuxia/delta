...
$req = mysql_query('SELECT id,name,size,norm,description,photo FROM gloves ORDER BY sequence LIMIT ' . $firstmessage . ', ' . $nbprodbypage); 
...
while ($data = mysql_fetch_array($req)){ 
 ...
 if($data['norm']==""){
 	...
 	echo '</td></tr><tr><td><img src="'.$data['norm'].$data['id'].'_ce.jpg" /></td></tr>';
 }
...
for ($i = 1 ; $i < $num ; $i++) { 
echo '<a href="index.php?dir=catalog&page=gloves&num=' . $i . '">'. $i .'</a> &nbsp;|&nbsp; '; 
} 
echo '<a href="index.php?dir=catalog&page=gloves&num=' . $num . '"><b><font size="+1">'. $num .'</font></b></a> &nbsp;|&nbsp; ';
